...
const NewSummaryForm = ({ user, successPost, setSuccess: setSuccessPost }) => {
  ...
  return (
    <>
      {fileError && (
        <BadFileType close={closeModal} message={'File format is not allowed, please add a different file that is allowed.'} />
      )}

      <form onSubmit={handleSubmit(onSubmit)}>
        <Box p={{ base: 0, lg: 5 }} pb={9}>
          <VStack spacing={10} align="stretch">
            <Box>
              <Heading as="h4" fontSize="1.4rem" color="purple.900">
                New Contribution<Badge ml={2}>Posting as &apos;{user.name}&apos;</Badge>
              </Heading>
              <Text mt={1}>Fill out all the fields, make sure to be informative and short as possible.</Text>
            </Box>

            <InputGroup flexDirection={'column'}>
              <FormLabel fontWeight={'bold'} htmlFor="title" color="purple.900" fontSize="1.25rem">
                Title:
              </FormLabel>
              <Controller
                name="title"
            ...
            <InputGroup flexDirection={'column'}>
              <FormLabel w={'max-content'} fontWeight={'bold'} htmlFor="description" color="purple.900" fontSize="1.25rem">
                Description:
              </FormLabel>
              <Controller
                name="description"
            ...
            <InputGroup flexDirection={'column'}>
              <Heading fontSize="1.25rem" color="purple.900">
                Upload Files
              </Heading>
              ...
              <Text as="sub" mt={3}>
                Allowed formats: *.pptx, *.docx *.xls *.xlsx *.doc *.pdf
              </Text>
            </InputGroup>
            ...
            <InputGroup flexDirection={'column'}>
              <Heading mb={1} color="purple.900" fontSize="1.25rem" display="block">
                Privacy Settings<Badge ml={2}>Access &amp; Visibility</Badge>
              </Heading>
              <Flex flexDirection={'row'} alignItems="center" justifyContent={'space-between'}>
                ...
                <Controller
                  ...
                  name="isLocked"
                  render={({ field }) => (
                    <Switch defaultChecked={false} {...field} mr={5} id="isLocked" size="lg" colorScheme={'purple'} />
                  )}
                />
              </Flex>
            </InputGroup>
